...
def create_poster_json(root_path, output_filename="posters", type_directory= "posters"):
    ...
    for category_name in sorted(os.listdir(directory_type_path)):
        ...
        for file_name in sorted(os.listdir(category_path)):
            ...
            placeholder_image_path = Path(item_folder_path) / f'''Holder.jpg'''
            ...
            if os.path.isdir(item_folder_path) and os.path.isfile(placeholder_image_path) and os.path.exists(placeholder_csv_path):
                ...
                item_data = {
                    "placeHolder": {
                        ...
                        "placeHolderUrl": f"{category_name}/{item_name}/Holder.jpg".replace(os.sep, '/'),
                        "placeHolderWidth": int(width/3),
                        "placeHolderHeight": int(height/3)
                    },
                    "zipFile": f"{category_name}/{file_name}".replace(os.sep, '/'),
...
if __name__ == "__main__":
    ...
    #    This is the directory that contains the category folders like "Abstract".
    # dir = os.getcwd()

    # directory_to_scan = Path("json_project/LogoMaker/posters") 

    # 2. Set the desired name for your output file
    # output_file = "my_posters.json"

    # 3. Run the function
    # create_poster_json(directory_to_scan, output_file)